...
vector<pair<vector<string>, vector<string>>> hands = {
...
bool royal_flush(vector<string> hand) {
    set<char> suit;
    for(auto x : hand) suit.insert(x[1]);
    if(suit.size() != 1) return false;
    string num = "";
    for(auto x : hand) num.push_back(x[0]);
    sort(num.begin(), num.end(), comp);
    return num == "TJQKA";
}

bool straight_flush(vector<string> hand) {
    set<char> suit;
    for(auto x : hand) suit.insert(x[1]);
    if(suit.size() != 1) return false;
    string num = "";
    for(auto x : hand) num.push_back(x[0]);
    sort(num.begin(), num.end(), comp);
    return (order.find(num) != string::npos);
}

pair<bool, char> quads(vector<string> hand) {
    map<char, int> mp;
    for(auto x : hand) mp[x[0]]++;
    for(auto x : mp){
        if(x.second == 4) {
            return {true, x.first};
        }
...
pair<bool, pair<char, char>> full_house(vector<string> hand) {
    map<char, int> mp;
    for(auto x : hand) mp[x[0]]++;
    pair<bool, pair<char, char>> ret = {false, {'-', '-'}};
    if(mp.size() != 2) return ret;
    for(auto x : mp){
        if(x.second == 2) {
            ret.first = true;
            ret.second.second = x.first;
        } else if(x.second == 3) {
            ret.first = true;
            ret.second.first = x.first;
        }
    }
    return ret;
}

pair<bool, char> flush(vector<string> hand) {
    set<char> suit;
    for(auto x : hand) suit.insert(x[1]);
    if(suit.size() != 1) return {false, '-'};
    return {true, (hand.back())[0]};
}

pair<bool, char> straight(vector<string> hand){
    string num = "";
    for(auto x : hand) num.push_back(x[0]);
    sort(num.begin(), num.end(), comp);
    if(num == "2345A") return {true, '5'};
    if(order.find(num) == string::npos) return {false, '-'};
    return {true, (hand.back())[0]};
}

pair<bool, char> three(vector<string> hand){
    map<char, int> mp;
    for(auto x : hand){
        mp[x[0]]++;
        if(mp[x[0]] == 3) return {true, x[0]};
    }
    return {false, '-'};
}

pair<bool, pair<char, char>> two_pair(vector<string> hand){
    map<char, int> mp;
    pair<bool, pair<char, char>> ret = {false, {'-', '-'}};
    for(auto x : hand){
        mp[x[0]]++;
        if(mp[x[0]] == 2 && ret.second.first == '-') ret.second.first = x[0];
        else if(mp[x[0]] == 2 && ret.second.second == '-') ret.second.second = x[0]; 
    }
    if(ret.second.first != '-' && ret.second.second != '-') ret.first = true;
    else ret = {false, {'-', '-'}};
    return ret;
}

pair<bool, char> pairs(vector<string> hand) {
    map<char, int> mp;
    for(auto x : hand) {
        mp[x[0]]++;
        if(mp[x[0]] == 2) return {true, x[0]};
    }
    return {false, '-'};
}

bool p1_winner(vector<string> &hand1, vector<string> &hand2) {
    sort(hand1.begin(), hand1.end(), comp2);
    sort(hand2.begin(), hand2.end(), comp2);
    bool rfh1 = royal_flush(hand1);
    bool rfh2 = royal_flush(hand2);
    if (rfh1) {
        return true;
    } else if (rfh2) {
        return false;
    ...
    bool sfh1 = straight_flush(hand1);
    bool sfh2 = straight_flush(hand2);
    if(sfh1 && sfh2) {
        if(val[(hand1.back())[0]] > val[(hand2.back())[0]]) return true;
        if(val[(hand1.back())[0]] < val[(hand2.back())[0]]) return false;
    } else if(sfh1) {
        return true;
    } else if(sfh2) {
        return false;
    ...
    pair<bool, char> quadh1 = quads(hand1);
    pair<bool, char> quadh2 = quads(hand2);
    if(quadh1.first && quadh2.first) {
        if(val[quadh1.second] > val[quadh2.second]) return true;
        if(val[quadh1.second] < val[quadh2.second]) return false;
        else {
            for(int i = hand1.size() - 1; i>-1; i--){
                if(val[hand1[i][0]] > val[hand2[i][0]]) return true;
                else if(val[hand1[i][0]] < val[hand2[i][0]]) return false;
            }
        }
    } else if(quadh1.first) {
        return true;
    } else if(quadh2.first) {
        return false;
    }
    
    pair<bool, pair<char, char>> fhh1 = full_house(hand1);
    pair<bool, pair<char, char>> fhh2 = full_house(hand2);
    if(fhh1.first && fhh2.first) {
        if(val[fhh1.second.first] > val[fhh2.second.first]) return true;
        else if(val[fhh1.second.first] < val[fhh2.second.first]) return false;
        else {
            if(val[fhh1.second.second] > val[fhh2.second.second]) return true;
            else if(val[fhh1.second.second] < val[fhh2.second.second]) return false;
    ...
    pair<bool, char> flush_h1 = flush(hand1);
    pair<bool, char> flush_h2 = flush(hand2);
    if(flush_h1.first && flush_h2.first) {
        if(val[flush_h1.second] > val[flush_h2.second]) return true;
        else if(val[flush_h1.second] < val[flush_h2.second]) return false;
        
    } else if(flush_h1.first) {
        return true;
    } else if(flush_h2.first) {
        return false;
    }
    
    pair<bool, char> straight_h1 = straight(hand1);
    pair<bool, char> straight_h2 = straight(hand2);
    if(straight_h1.first && straight_h2.first) {
        if(val[straight_h1.second] > val[straight_h2.second]) return true;
        else if(val[straight_h1.second] < val[straight_h2.second]) return false;
        
    } else if(straight_h1.first) {
        return true;
    } else if(straight_h2.first) {
        return false;
    }
    
    pair<bool, char> three_h1 = three(hand1);
    pair<bool, char> three_h2 = three(hand2);
    if(three_h1.first && three_h2.first) {
        if(val[three_h1.second] > val[three_h2.second]) return true;
        else if(val[three_h1.second] < val[three_h2.second]) return false;
        else{
            for(int i = hand1.size() - 1; i>-1; i--){
                if(val[hand1[i][0]] > val[hand2[i][0]]) return true;
                else if(val[hand1[i][0]] < val[hand2[i][0]]) return false;
    ...
    } else if(three_h1.first) {
        return true;
    } else if(three_h2.first) {
        return false;
    }
    
    pair<bool, pair<char, char>> two_pair_h1 = two_pair(hand1);
    pair<bool, pair<char, char>> two_pair_h2 = two_pair(hand2);
    if(two_pair_h1.first && two_pair_h2.first) {
        if(val[two_pair_h1.second.second] > val[two_pair_h2.second.second]) return true;
        else if(val[two_pair_h1.second.second] < val[two_pair_h2.second.second]) return false;
        else {
            if(two_pair_h1.second.first > two_pair_h2.second.first) return true;
            else if(two_pair_h1.second.first < two_pair_h2.second.first) return false;
            else {
                for(int i = hand1.size() - 1; i>-1; i--){
                    if(val[hand1[i][0]] > val[hand2[i][0]]) return true;
                    else if(val[hand1[i][0]] < val[hand2[i][0]]) return false;
    ...
    } else if (two_pair_h1.first) {
        return true;
    } else if (two_pair_h2.first) {
        return false;
    }
    
    pair<bool, char> pair_h1 = pairs(hand1);
    pair<bool, char> pair_h2 = pairs(hand2);
    if(pair_h1.first && pair_h2.first) {
        if(val[pair_h1.second] > val[pair_h2.second]) return true;
        else if(val[pair_h1.second] < val[pair_h2.second]) return false;
        else{
            for(int i = hand1.size() - 1; i>-1; i--){
                if(val[hand1[i][0]] > val[hand2[i][0]]) return true;
                else if(val[hand1[i][0]] < val[hand2[i][0]]) return false;
    ...
    } else if(pair_h1.first) {
        return true;
    } else if(pair_h2.first) {
        return false;
    }
